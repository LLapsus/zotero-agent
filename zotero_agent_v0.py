#!/usr/bin/env python3
"""
zotero_agent_v0.py  -  LEGACY: the TWO-TOOL version. Kept for comparison.

The current zotero_agent.py replaces the two separate tools below with one
unified search_library(author?, topic?) tool, so the model fills parameters
instead of choosing between overlapping tools -- and "author AND topic" becomes
a real filtered search instead of a guess. Read this first, then diff it against
zotero_agent.py to see why one tool with optional filters composes better.
